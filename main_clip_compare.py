# main_clip_compare.py
from utils.downloader import download_video
from utils.frame_extractor import extract_frame
from utils.image_utils import get_clip_embedding, cosine_sim
import os
...
for t in timestamps:
    ...
    emb1 = get_clip_embedding(f1)
    emb2 = get_clip_embedding(f2)
    similarity = cosine_sim(emb1, emb2)
    print(f"Similarity at {t}s: {similarity * 100:.2f}%")
    total += similarity
    os.remove(f1)
    os.remove(f2)

avg = total / len(timestamps)
print(f"Average Similarity: {avg * 100:.2f}%")
if avg > 0.7:
